package com.example.chewsyui.phoneLoginScreen
...
import com.example.chewsyui.login.AuthRepository

class PhoneLoginFragment : Fragment() {

    private val model by viewModels<PhoneLoginViewModel> {
        PhoneLoginViewModelFactory(requireActivity().application, AuthRepository.getInstance())
    }
    ...
    private fun getScreen() = ComposeView(requireContext()).apply {
        setContent { PhoneLoginScreen(model = model, requireActivity()) }
    }
    ...
    private fun navigateToHome() {
        findNavController().navigate(PhoneLoginFragmentDirections.actionGlobalHomeFragment())
    }
